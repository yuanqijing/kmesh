...
static inline int rate_limit__check_and_take(struct ratelimit_key *key, const struct ratelimit_settings *settings)
{
    ...
    value = bpf_map_lookup_elem(&ratelimit_map, key);
    if (!value) {
        new_value.last_topup = now;
        new_value.tokens = settings->bucket_size;
        bpf_map_update_elem(&ratelimit_map, key, &new_value, BPF_ANY);
        return 0;
    }
...
static inline int Local_rate_limit__filter_chain__match(const Listener__FilterChain *filter_chain, Listener__Filter **filter_ptr);
